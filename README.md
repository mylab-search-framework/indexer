...
Файлы реусурсов индексов распологаются в директории, указанной в настройках индексатора в поле `ResourcePath`. По умолчанию `/etc/mylab-indexer/indexes` . Это общая диреткория ресурсов.

Индексатор ищет файлы ресурсов индексов в поддиректории с  имененм, соответствующим идентифиактору индекса - в персональных директориях индекстов. 
...
Файл `index.json` может быть указан как для конкретного индекса в персональной диретории, так и общий для всех индексов в общей диреткории. Если указаны оба, то файлы будут обединяться. При этом при конфликтах объекдинения персональные настройки будут иметь приоритет.
