...
* `lifecycle-policies` -  директория, содержащая файлы политик жизненных циклов;
* `index-templates` -  директория, содержащая файлы шаблонов индексов;
...
Директория для размещения файлов политик жизненных циклов индексов. Содержательная часть файла должна соответствовать [запросу создания/обновления политики жизненного цикла в Elasticsearch](https://www.elastic.co/guide/en/elasticsearch/reference/7.17/ilm-put-lifecycle.html). А имя должно соответствовать идентификатору этой политики, которое будет использоваться при её создании. 
...
При каждом запуске индексатор будет проверять наличие этих политик в Elasticsearch и при несовпадении версии, при наличии пометки, что политика была создан автоматически, будет обновлять его. А при отсутствии - создавать.
...
Кроме того, при разработке политики следует снабдить её пометкой номера версии: `mylab:ver`. При очередном перезапуске, политика будет обновлена, если версия отличается от аналогичного значения в `Elasticsearch`.

Тогда политика в `Elasticsearch` будет выглядеть так (фазы урезаны для наглядности):
...
*Более подробно о [политиках жизненных циклов индексов](https://www.elastic.co/guide/en/elasticsearch/reference/7.17/ilm-index-lifecycle.html)*.
