...
* происходит порциональная выборка данных и по каждой порции:
  ...
  * документы индексируются в индексе `Elasticsearch`, имя которго передаётся в запросе или указано в настройках *индекса* в поле `EsIndex`   
...
* определение имени индекса в `Elasticsearch`, которое берётся из запроса или настроек индекса из поля `EsIndex`;
