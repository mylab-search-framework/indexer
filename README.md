...
* происходит порциональная выборка данных и по каждой порции:
  ...
  * документы индексируются в индексе `Elasticsearch`, имя которго указано в настройках *индекса* в поле `EsIndex`   
...
* определение имени индекса в `Elasticsearch`, которое берётся из настроек индекса из поля `EsIndex`;
...
* `Indexes` - настройки *индексов*:
  ...
  * `EsIndex` - имя целевого индекса в `Elasticsearcch` (будет переведён в нижний регистр).
