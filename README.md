...
Приложение `Indexer` определяет абстракцию `Index`, объединяющую настроки индексации сущностей из БД, очереди и `api`. `Indexer` через конфигурацию поддерживает указание нескольких `Index`, что позволяет использовать один экземпляр `Indexer` для индекксирования нескольких типов сущностей, которые извлекаются разными запросами, получаются из разных очередей и индексируются в разных индексах по разным правилам.

Все объявленные `Index`-ы синхронизируются последовательно, в соответствии с конфигурацией в рамках каждой итерации. В случае получения сообщений из очереди, `Index` определяется по очереди, из которой получено сообщение. При обращзении через `API` имя `Index` передаётся в запросе.
## Индексация через БД
...
* конфигурации `Index`-а: `Indexer.Indexes[].DbQuery`

* или размещён в файле, путь которого состоит из следующих частей:

  * базовый путь к директории с файлами `Index`-ов из конфигурации `Indexer.IndexPath` = `/etc/mylab-indexer/indexes` (по умолчанию);
  ...
  * `sync.sql`.
  ...
  ```
  /etc/mylab-indexer/indexes/users/sync.sql
  ```
...
При `kick`-индексации, среди прочих, необходимо указывать следующие параметры для `Index`:

* `IdPropertyType`
* `KickDbQuery` или файл `kick.sql` в ресурсах `Index`-а.
...
* `IndexesPath` - базовый путь к директориям ресурсов `Index`-ов. По умолчанию - `/etc/mylab-indexer/indexes`;
* `SeedPath` - базовый путь к директории, где будут храниться `seed`-ы `Index`-ов;
* `EsIndexNamePrefix` - префикс, который будет добавляться к имени индекса `Elasticsearch` всех `Index`-ов (должен быть в нижнем регистре);
* `EsIndexNamePostfix` - постфикс, который будет добавляться к имени индекса `Elasticsearch` всех `Index`-ов (должен быть в нижнем регистре);
* `Indexes` - настройки `Index`-ов:
  * `Id` - литеральный идентификатор `Index`-а. Например, `users` при индексации информации о пользователях;
