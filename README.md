...
* `IndexNamePrefix` - префикс, который будет добавляться к имени индекса всех джобов (должен быть в нижнем регистре);
* `IndexNamePostfix` - постфикс, который будет добавляться к имени индекса всех джобов (должен быть в нижнем регистре);
